...
from app.users.schemas import UserCreate, UserLogin, UserResponse, TokenSchema, PasswordChangeSchema
from app.core.security import get_password_hash, verify_password, create_access_token, create_refresh_token, decode_token
from app.common.exceptions import ConflictException, UnauthorizedException, NotFoundException, BadRequestException
...
class AuthService:
    ...
    def refresh_access_token(self, refresh_token_str: str) -> TokenSchema:
        try:
            ...
            sub = payload.get("sub")
            if not sub:
                raise BadRequestException("Missing sub in refresh token payload")
            user_id = UUID(sub)
